/**
 * 《反着来》题库适配层
 * 依赖：src/data/questionPool/*Questions.js
 */
const QuestionBank = (function (global) {
  ...
  function sampleStable(difficulty, count) {
    ...
    for (var i = 0; i < count; i++) {
      selected.push(normalizeQuestion(shuffled[i % shuffled.length]));
    }
    return selected;
  }
  ...
  function getLevelQuestions(difficulty, count, timeLimit) {
    if (!pools[difficulty]) return [];
    return sampleStable(difficulty, count || 10).map(function (question) {
